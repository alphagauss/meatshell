...
async fn run_session(
    ...
    // includes ignorespace/ignoreboth; the echoed command is also stripped
    // locally via ECHO_NEEDLE below.
    const PROMPT_SETUP: &[u8] = b" export PROMPT_COMMAND='printf \"\\033]7;file://${HOSTNAME}${PWD}\\007\"' && eval \"$PROMPT_COMMAND\"\r";
    const ECHO_NEEDLE: &str =
        "export PROMPT_COMMAND='printf \"\\033]7;file://${HOSTNAME}${PWD}\\007\"' && eval \"$PROMPT_COMMAND\"";
