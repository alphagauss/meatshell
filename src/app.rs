...
type TransferWindows = Rc<RefCell<Vec<TransferWindowState>>>;
...
fn wire_app_state_callbacks(
...

    {
        ...
        let runtime = runtime.clone();
        let store = store.clone();
        let transfer_windows = transfer_windows.clone();
        window.on_open_transfer_window(move || {
            ...
            match open_transfer_window(
                ...
                Err(err) => set_terminal_row(&w, &active, |row| {
                    row.status =
                        format!("{}: {err:#}", t("打开文件传输失败", "Open transfer failed"))
                            .into();
                }),
            }
        });
...
fn wire_transfer_window_callbacks(window: &TransferWindow, sftp: Rc<SftpHandle>) {
    {
        let weak = window.as_weak();
...
fn refresh_transfer_local(window: &TransferWindow, path: impl AsRef<std::path::Path>) {
...
fn local_entries_model(entries: Vec<LocalFileEntry>) -> ModelRc<SftpEntry> {
...
fn remote_entries_model(entries: Vec<RemoteEntry>) -> ModelRc<SftpEntry> {
    let rows: Vec<SftpEntry> = entries
        .into_iter()
        .map(|entry| SftpEntry {
            name: entry.name.into(),
...
fn spawn_transfer_sftp_event_pump(
...
fn apply_transfer_event_to_window(window: &TransferWindow, event: SessionEvent) {
    match event {
